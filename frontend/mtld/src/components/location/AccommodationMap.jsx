...
import dummy from 'data/AccommodationData.json';
...
`;
const StyledCardTravelIcon = styled(CardTravelIcon)`
  // color: #646464;
          ...
          if (searchPlace === '' && flag === false) {
            ...
            function displayMarker(place) {
              ...
              if (place.img === undefined) {
                place.img = 'https://media.istockphoto.com/photos/dog-in-airport-terminal-on-vacation-picture-id667787320?k=20&m=667787320&s=612x612&w=0&h=d0rFvtiyusYz-4fvn8J2xC8ySZRWWEIAyQ2V_SfT_Eg=';
              }
              // console.log('place = ', place);
            ...
            const infowindow = new kakao.maps.InfoWindow({ zIndex: 1 });
            const ps = new kakao.maps.services.Places(); // 장소 검색 객체 생성
            ps.keywordSearch(searchPlace.concat('반려견동반숙소'), placeSearchCB);
            function placeSearchCB(data, status, pagination) {
              if (status === kakao.maps.services.Status.OK) {
                const bounds = new kakao.maps.LatLngBounds();
                for (let i = 0; i < data.length; i += 1) {
            ...
            <div key={i}>
              <ResultItem>
                <ItemDiv>
                  <PlaceDiv>
                    <div>
                      <StyledCardTravelIcon fontSize="small" />
